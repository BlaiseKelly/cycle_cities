...
# create download dir for osm files
dir.create("X:/osm_temp/")
...
# import urban audit city shapes
urban_audit <- gisco_get_urban_audit(year = 2014,level = "CITIES") |>
  transmute(name = URAU_NAME, geometry) |>
  st_transform(4326)


harrisonburg <- st_read("CityBoundaries_shp/CityBoundaries.shp") |>
  transmute(name = "Harrisonburg", geometry) |>
  st_transform(4326)

urban_audit <- rbind(urban_audit,harrisonburg)
...
for (c in cities2plot){
  ...
  rm(area,cycle_net, drive_net, seg_paths,cycle_net_c)

  area <- urban_audit[urban_audit$name == c,]
    ...
    tm1 <- tm_shape(bg_m)+
      ...
      tm_title(text = c, position = c(0,0.99))+
      tm_credits(paste0("Area population (2024): ", format(round(city_data2$city_pop), big.mark = ",", scientific = FALSE), "\n",
...
city_table <- do.call(rbind,city_list)

saveRDS(city_table, "city_table.RDS")
